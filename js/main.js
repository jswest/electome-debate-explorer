...
var timeframes = {
	debate1: 'During the first presidential debate',
	debate2: 'During the vice-presidential debate',
	debate3: 'During the second presidential debate'
};
...
d3.json( 'data/data-1.json', function ( response1 ) {
	d3.json( 'data/data-2.json', function ( response2 ) {
		d3.json( 'data/data-3.json', function ( response3 ) {
			...
			d3.select( '#viz-topics-dropdown' ).selectAll( 'li' )
				.data( _.keys( response.debate1.during.all ).sort() )
				.enter()
				.append( 'li' )
					...
					_.each( activeExclusives, function ( exclusive, j ) {
						_.each( activeTopics, function ( topic, k ) {
							data.push( {
								...
								exclusive: exclusive,
								topic: topic,
								value: response[timeframe].during[exclusive][topic]
							} );
						} );
					} );
				} );
